...
if (!function_exists('update_fields')) {
    function update_fields($table, array $fields, $whereField = null, $whereValue = null, array $options = [])
    {
        global $pdo, $user;
        ...
        $skipLogFields = $options['skip_log_fields'] ?? ['message_count', 'last_message_time'];
        ...
        foreach ($fields as $field => $newValue) {
            ...
            $setClauses[] = "$field = ?";
            $values[] = $valueToStore;
            $normalizedFields[$field] = $valueToStore;
        }

        $whereValueToStore = null;
        if ($whereField !== null) {
            $whereValueToStore = normaliseUpdateValue($whereValue);
            ensureColumnExistsForUpdate($table, $whereField, $whereValueToStore);
            $values[] = $whereValueToStore;
        }

        $executeUpdate = function (array $updateValues) use ($pdo, $table, $setClauses, $whereField) {
        ...
        try {
            $affectedRows = $executeUpdate($values);
        } catch (PDOException $e) {
            if (strpos($e->getMessage(), 'Incorrect string value') !== false) {
                $tableConverted = ensureTableUtf8mb4($table);
                if ($tableConverted) {
                    ...
                    foreach ($values as $value) {
                        if (is_string($value)) {
                            $fallbackValue = @iconv('UTF-8', 'UTF-8//IGNORE', $value);
                            $sanitizedValues[] = $fallbackValue === false ? '' : $fallbackValue;
                        } else {
                            $sanitizedValues[] = $value;
                        }
                    }
                    $affectedRows = $executeUpdate($sanitizedValues);
                }
            } else {
                throw $e;
            }
        ...
        if ($whereField !== null && $affectedRows === 0) {
            ...
            if ($rowExists === false) {
                $columns = array_keys($normalizedFields);
                $insertValues = array_values($normalizedFields);

                if ($whereField !== null && !array_key_exists($whereField, $normalizedFields)) {
                    $columns[] = $whereField;
                    $insertValues[] = $whereValueToStore;
                }
                ...
                try {
                    ...
                    error_log('Failed to insert missing row during update_fields fallback: ' . $insertException->getMessage());
                }
            }
        }

        $loggableFields = array_diff(array_keys($normalizedFields), $skipLogFields);
        if ($logEnabled && !empty($loggableFields)) {
            $date = date("Y-m-d H:i:s");
            if (!isset($user['step'])) {
                $user['step'] = '';
            }
            $logValue = json_encode($normalizedFields, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
            $logss = "{$table}_batch_{$logValue}_{$whereField}_{$whereValue}_{$user['step']}_$date";
            file_put_contents('log.txt', "\n" . $logss, FILE_APPEND);
        }
...
if (!function_exists('userExists')) {
    function userExists($userId)
    {
        ...
        $row = select('user', 'id', 'id', $userId, 'select');
        $exists = is_array($row) && isset($row['id']);
